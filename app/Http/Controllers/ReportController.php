...
class ReportController extends Controller
{
    ...
    public function getStockBySellingPrice(Request $request)
    {
        ...
        $day_before_start_date = \Carbon::createFromFormat("Y-m-d", $start_date)
            ->subDay()
            ->format("Y-m-d");
        ...
        $opening_stock_by_sp = $this->transactionUtil->getOpeningClosingStock(
            $business_id,
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $location_id = !empty(request()->input("location_id"))
                ? request()->input("location_id")
                : null;
            $start_date = !empty(request()->input("start_date"))
                ? request()->input("start_date")
                : $fy["start"];
            $end_date = !empty(request()->input("end_date"))
            ...
            return view(
                "report.partials.profit_loss_details",
                compact("data")
            )->render();
        }
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_sell_return_inc_tax =
                $transaction_totals["total_sell_return_inc_tax"];

            $difference = [
                "total" =>
                    $sell_details["total_sell_inc_tax"] -
                    $total_sell_return_inc_tax -
                    ($purchase_details["total_purchase_inc_tax"] -
                        $total_purchase_return_inc_tax),
                "due" =>
                    $sell_details["invoice_due"] -
                    $purchase_details["purchase_due"],
            ];
        ...
        return view("report.purchase_sell")->with(
            compact("business_locations")
        );
    }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        if ($request->ajax()) {
            $contacts = Contact::where("contacts.business_id", $business_id)
                ...
                ->select(
                    ...
                    DB::raw(
                        "SUM(IF(t.type = 'purchase', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_paid"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as invoice_received"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'sell_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as sell_return_paid"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'purchase_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_return_received"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'sell_return', final_total, 0)) as total_sell_return"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'opening_balance', final_total, 0)) as opening_balance"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'ledger_discount' AND sub_type='sell_discount', final_total, 0)) as total_ledger_discount_sell"
                    ),
                    DB::raw(
                        "SUM(IF(t.type = 'ledger_discount' AND sub_type='purchase_discount', final_total, 0)) as total_ledger_discount_purchase"
                    ),
            ...
            if (!empty($request->input("contact_type"))) {
                $contacts->whereIn("contacts.type", [
            ...
            return Datatables::of($contacts)
                ...
                ->addColumn("due", function ($row) {
                    $total_ledger_discount_purchase =
                        $row->total_ledger_discount_purchase ?? 0;
                    $total_ledger_discount_sell =
                        $total_ledger_discount_sell ?? 0;
                    $due =
                        $row->total_invoice -
                    ...
                    return '<span class="total_due" data-orig-value="' .
                        $due .
                        '">' .
                        $due_formatted .
                        "</span>";
        ...
        $contact_dropdown = Contact::contactDropdown(
            $business_id,
            false,
            false
        );

        return view("report.contact")->with(
            compact(
                "customer_group",
                "types",
                "business_locations",
    ...
    public function getStockReport(Request $request)
    {
        ...
        $selling_price_groups = SellingPriceGroup::where(
            "business_id",
            $business_id
        )->get();
        ...
        foreach ($selling_price_groups as $selling_price_group) {
            if (
                auth()
                    ->user()
                    ->can("selling_price_group." . $selling_price_group->id)
            ) {
        ...
        if ($request->ajax()) {
            ...
            $filters["not_for_selling"] =
                isset($filters["not_for_selling"]) &&
                $filters["not_for_selling"] == "true"
                    ? 1
                    : 0;
            ...
            //Return the details in ajax call
            $for =
                request()->input("for") == "view_product"
                    ? "view_product"
                    : "datatables";

            $products = $this->productUtil->getProductStockDetails(
                $business_id,
                $filters,
                $for
            );
            ...
            if (
                ...
                return view("product.partials.product_stock_details")->with(
            ...
            $datatable = Datatables::of($products)
                ->editColumn("stock", function ($row) {
                    if ($row->enable_stock) {
                        ...
                        return '<span class="current_stock" data-orig-value="' .
                            (float) $stock .
                            '" data-unit="' .
                            $row->unit .
                            '"> ' .
                ...
                ->addColumn("action", function ($row) {
                    return '<a class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info tw-w-max " href="' .
                        ...
                        __("lang_v1.product_stock_history") .
                        "</a>";
                })
                ...
                ->editColumn("total_sold", function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="total_sold" data-orig-value="' .
                        $total_sold .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                ...
                ->editColumn("total_transfered", function ($row) {
                    ...
                    return '<span class="total_transfered" data-orig-value="' .
                        $total_transfered .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                ...
                ->editColumn("total_adjusted", function ($row) {
                    ...
                    return '<span class="total_adjusted" data-orig-value="' .
                        $total_adjusted .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                ...
                ->editColumn("unit_price", function ($row) use (
                    $allowed_selling_price_group
                ) {
                    $html = "";
                    if (
                        auth()
                        ...
                        $html .= $this->transactionUtil->num_f(
                            $row->unit_price,
                            true
                        );
                    }
                    ...
                    if ($allowed_selling_price_group) {
                        $html .=
                            ' <button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary tw-w-max btn-modal no-print" data-container=".view_modal" data-href="' .
                            action(
                                [
                                    \App\Http\Controllers\ProductController::class,
                                    "viewGroupPrice",
                ...
                ->editColumn("stock_price", function ($row) {
                    $html =
                        '<span class="total_stock_price" data-orig-value="' .
                        $row->stock_price .
                        '">' .
                        $this->transactionUtil->num_f($row->stock_price, true) .
                        "</span>";
                ...
                ->editColumn("stock_value_by_sale_price", function ($row) {
                    $stock = $row->stock ? $row->stock : 0;
                    $unit_selling_price =
                        (float) $row->group_price > 0
                            ? $row->group_price
                            : $row->unit_price;
                    $stock_price = $stock * $unit_selling_price;

                    return '<span class="stock_value_by_sale_price" data-orig-value="' .
                        (float) $stock_price .
                        '" > ' .
                        $this->transactionUtil->num_f($stock_price, true) .
                        "</span>";
                })
                ->addColumn("potential_profit", function ($row) {
                    ...
                    $potential_profit =
                        (float) $stock_price_by_sp - (float) $row->stock_price;

                    return '<span class="potential_profit" data-orig-value="' .
                        (float) $potential_profit .
                        '" > ' .
                        $this->transactionUtil->num_f($potential_profit, true) .
                        "</span>";
                })
                ->setRowClass(function ($row) {
                    return $row->enable_stock &&
                    $row->stock <= $row->alert_quantity
                        ? "bg-danger"
                        : "";
                })
            ...
            if ($show_manufacturing_data) {
                $datatable->editColumn("total_mfg_stock", function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="total_mfg_stock"  data-orig-value="' .
                        $total_mfg_stock .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
        ...
        $units = Unit::where("business_id", $business_id)->pluck(
            "short_name",
            "id"
        );
        $business_locations = BusinessLocation::forDropdown($business_id, true);
    ...
    public function getStockDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view("report.stock_details")->with(
        ...
        if ($request->ajax()) {
            ...
            if ($type == "sell") {
                $sells
                    ...
                    ->where(function ($query) {
                        $query
                            ->whereHas("sell_lines", function ($q) {
                                $q->whereNotNull(
            ...
            if ($type == "purchase") {
                $sells
                    ...
                    ->where(function ($query) {
                        $query
                            ->whereHas("purchase_lines", function ($q) {
            ...
            foreach ($group_taxes_array as $group_tax) {
                foreach ($group_tax["sub_taxes"] as $sub_tax) {
                    $group_taxes[$group_tax->id]["sub_taxes"][
                    $sub_tax->id
                    ] = $sub_tax;
                }
            }
            foreach ($taxes as $tax) {
                $col = "tax_" . $tax["id"];
                $raw_cols[] = $col;
                $datatable->addColumn($col, function ($row) use (
                    $tax,
                    $type,
                    $col,
                    $group_taxes
                    ...
                    if ($type == "sell") {
                        foreach ($row->sell_lines as $sell_line) {
                            ...
                            //break group tax
                            if (
                                $sell_line->line_tax->is_tax_group == 1 &&
                                array_key_exists(
                                    $tax["id"],
                                    $group_taxes[$sell_line->tax_id][
                                    "sub_taxes"
                                    ]
                                )
                            ) {
                                $group_tax_details = $this->transactionUtil->groupTaxDetails(
                                    $sell_line->line_tax,
                                    $sell_line->item_tax
                                );
                                ...
                                foreach (
                                    ...
                                    if ($sub_tax_details["id"] == $tax["id"]) {
                                        $sub_tax_share =
                                            $sub_tax_details["calculated_tax"];
                                    }
                                }
                        ...
                        foreach ($row->purchase_lines as $purchase_line) {
                            ...
                            //break group tax
                            if (
                                $purchase_line->line_tax->is_tax_group == 1 &&
                                array_key_exists(
                                    $tax["id"],
                                    $group_taxes[$purchase_line->tax_id][
                                    "sub_taxes"
                                    ]
                                )
                            ) {
                                $group_tax_details = $this->transactionUtil->groupTaxDetails(
                                    $purchase_line->line_tax,
                                    $purchase_line->item_tax
                                );
                    ...
                    //break group tax
                    if (
                        !empty($group_taxes[$row->tax_id]) &&
                        array_key_exists(
                            $tax["id"],
                            $group_taxes[$row->tax_id]["sub_taxes"]
                        )
                    ) {
                        $group_tax_details = $this->transactionUtil->groupTaxDetails(
                            $row->tax_id,
                    ...
                    if ($tax_amount > 0) {
                        return '<span class="display_currency ' .
                            $col .
                            '" data-currency_symbol="true" data-orig-value="' .
                            $tax_amount .
                            '">' .
                            $tax_amount .
                            "</span>";
            ...
            $datatable
                ...
                ->editColumn(
                    "contact_name",
                    '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$contact_name}}'
                )
                ->addColumn("payment_methods", function ($row) use (
                    $payment_types
                ) {
                    $methods = array_unique(
                        $row->payment_lines->pluck("method")->toArray()
                    );
                    $count = count($methods);
                    $payment_method = "";
                    if ($count == 1) {
                    ...
                    $html = !empty($payment_method)
                        ? '<span class="payment-method" data-orig-value="' .
                        $payment_method .
                        '" data-status-name="' .
                        $payment_method .
                        '">' .
            ...
            return $datatable->rawColumns($raw_cols)->make(true);
        }
    }
    ...
    public function getTaxReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $input_tax_details = $this->transactionUtil->getInputTax(
                $business_id,
                $start_date,
                $end_date,
                $location_id,
            ...
            $output_tax_details = $this->transactionUtil->getOutputTax(
                $business_id,
                $start_date,
                $end_date,
                $location_id,
            ...
            $expense_tax_details = $this->transactionUtil->getExpenseTax(
                $business_id,
                $start_date,
                $end_date,
                $location_id,
            ...
            $module_output_taxes = $this->moduleUtil->getModuleData(
                "getModuleOutputTax",
            ...
            $total_output_tax =
                $output_tax_details["total_tax"] + $total_module_output_tax;

            $tax_diff =
                $total_output_tax -
                $input_tax_details["total_tax"] -
                $expense_tax_details["total_tax"];
        ...
        $tax_report_tabs = $this->moduleUtil->getModuleData(
            "getTaxReportViewTabs"
        );
        ...
        return view("report.tax_report")->with(
            compact(
                "business_locations",
                "taxes",
                "tax_report_tabs",
    ...
    public function getTrendingProducts(Request $request)
    {
        ...
        if (!empty($date_range)) {
            ...
            $filters["start_date"] = $this->transactionUtil->uf_date(
                trim($date_range_array[0])
            );
            $filters["end_date"] = $this->transactionUtil->uf_date(
                trim($date_range_array[1])
            );
        }

        $products = $this->productUtil->getTrendingProducts(
            $business_id,
            $filters
        );
        ...
        return view("report.trending_products")->with(
            compact(
                "chart",
                "categories",
                "brands",
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        $expenses = $this->transactionUtil->getExpenseReport(
            $business_id,
            $filters
        );
        ...
        return view("report.expense_report")->with(
            compact("chart", "categories", "business_locations", "expenses")
        );
    }
    ...
    public function getStockAdjustmentReport(Request $request)
    {
        ...
        //Return the details in ajax call
        if ($request->ajax()) {
            $query = Transaction::where("business_id", $business_id)->where(
                "type",
                "stock_adjustment"
            );
            ...
            $stock_adjustment_details = $query
                ->select(
                    DB::raw("SUM(final_total) as total_amount"),
                    DB::raw("SUM(total_amount_recovered) as total_recovered"),
                    DB::raw(
                        "SUM(IF(adjustment_type = 'normal', final_total, 0)) as total_normal"
                    ),
                    DB::raw(
                        "SUM(IF(adjustment_type = 'abnormal', final_total, 0)) as total_abnormal"
                    )
                )
                ->first();
        ...
        return view("report.stock_adjustment_report")->with(
            compact("business_locations")
        );
    }
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $registers = $this->transactionUtil->registerReport(
                $business_id,
                $permitted_locations,
                $start_date,
                $end_date,
            ...
            return Datatables::of($registers)
                ->editColumn("total_card_payment", function ($row) {
                    return '<span data-orig-value="' .
                        $row->total_card_payment .
                        '" >' .
                        $this->transactionUtil->num_f(
                            $row->total_card_payment,
                            true
                        ...
                        " (" .
                        $row->total_card_slips .
                        ")</span>";
                })
                ->editColumn("total_cheque_payment", function ($row) {
                    return '<span data-orig-value="' .
                        $row->total_cheque_payment .
                        '" >' .
                        $this->transactionUtil->num_f(
                            $row->total_cheque_payment,
                            true
                ...
                ->editColumn("total_bank_transfer_payment", function ($row) {
                    return '<span data-orig-value="' .
                        $row->total_bank_transfer_payment .
                        '" >' .
                        $this->transactionUtil->num_f(
                            $row->total_bank_transfer_payment,
                            true
                ...
                ->editColumn("closed_at", function ($row) {
                    if ($row->status == "close") {
                        return $this->productUtil->format_date(
                            $row->closed_at,
                            true
                        );
                    } else {
                ...
                ->addColumn("total", function ($row) {
                    ...
                    return '<span data-orig-value="' .
                        $total .
                        '" >' .
                        $this->transactionUtil->num_f($total, true) .
                        "</span>";
                })
                ->addColumn(
                    "action",
                    '<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@show\', [$id])}}" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info tw-w-max btn-modal" 
                ...
                ->filterColumn("user_name", function ($query, $keyword) {
                    $query->whereRaw(
                        "CONCAT(COALESCE(surname, ''), ' ', COALESCE(first_name, ''), ' ', COALESCE(last_name, ''), '<br>', COALESCE(u.email, '')) like ?",
                        ["%{$keyword}%"]
                    );
                })
                ->rawColumns([
        ...
        return view("report.register_report")->with(
            compact("users", "payment_types")
        );
    }
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
        ...
        return view("report.sales_representative")->with(
            compact("users", "business_locations", "pos_settings")
        );
    }
    ...
    public function getSalesRepresentativeTotalExpense(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_expense = $this->transactionUtil->getExpenseReport(
                $business_id,
                $filters,
                "total"
            );
    ...
    public function getSalesRepresentativeTotalSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_sell_return = !empty(
            $sell_return_details["total_sell_return_exc_tax"]
            )
                ? $sell_return_details["total_sell_return_exc_tax"]
                : 0;
            $total_sell =
                $sell_details["total_sell_exc_tax"] - $total_sell_return;
    ...
    public function getSalesRepresentativeTotalCommission(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $commsn_calculation_type =
                empty($pos_settings["cmmsn_calculation_type"]) ||
                $pos_settings["cmmsn_calculation_type"] == "invoice_value"
                    ? "invoice_value"
                    : $pos_settings["cmmsn_calculation_type"];

            $commission_percentage = User::find($commission_agent)
                ->cmmsn_percent;
            ...
            if ($commsn_calculation_type == "payment_received") {
                ...
                $total_commission =
                    ($commission_percentage *
                        $payment_details["total_payment_with_commission"]) /
                    100;
            ...
            $sell_details = $this->transactionUtil->getTotalSellCommission(
                $business_id,
                $start_date,
                $end_date,
                $location_id,
    ...
    public function getStockExpiryReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $report = $query
                ->select(
                    ...
                    "u.short_name as unit",
                    DB::raw(
                        "SUM(COALESCE(quantity, 0) - COALESCE(quantity_sold, 0) - COALESCE(quantity_adjusted, 0) - COALESCE(quantity_returned, 0)) as stock_left"
                    ),
                    "t.ref_no",
            ...
            return Datatables::of($report)
                ...
                ->editColumn("ref_no", function ($row) {
                    return '<button type="button" data-href="' .
                        ...
                        '" class="btn btn-link btn-modal" data-container=".view_modal"  >' .
                        $row->ref_no .
                        "</button>";
                })
                ->editColumn("stock_left", function ($row) {
                    return '<span data-is_quantity="true" class="display_currency stock_left" data-currency_symbol=false data-orig-value="' .
                        $row->stock_left .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                ...
                ->addColumn("edit", function ($row) {
                    $html =
                        '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary stock_expiry_edit_btn" data-transaction_id="' .
                        $row->transaction_id .
                    ...
                    if (!empty($row->exp_date)) {
                        $carbon_exp = \Carbon::createFromFormat(
                            "Y-m-d",
                            $row->exp_date
                        );
                        $carbon_now = \Carbon::now();
                        if ($carbon_now->diffInDays($carbon_exp, false) < 0) {
                            $html .=
                                ' <button type="button" class="btn btn-warning btn-xs remove_from_stock_btn" data-href="' .
                                action(
                                    [
                                        \App\Http\Controllers\StockAdjustmentController::class,
                                        "removeExpiredStock",
                                    ],
                                    [$row->purchase_line_id]
                                ) .
                                '"> <i class="fa fa-trash"></i> ' .
                                __("lang_v1.remove_from_stock") .
                                "</button>";
                        }
                    }
        ...
        $view_stock_filter = [
            \Carbon::now()
                ->subDay()
                ->format("Y-m-d") => __("report.expired"),
            \Carbon::now()
                ->addWeek()
                ->format("Y-m-d") => __("report.expiring_in_1_week"),
            \Carbon::now()
                ->addDays(15)
                ->format("Y-m-d") => __("report.expiring_in_15_days"),
            \Carbon::now()
                ->addMonth()
                ->format("Y-m-d") => __("report.expiring_in_1_month"),
            \Carbon::now()
                ->addMonths(3)
                ->format("Y-m-d") => __("report.expiring_in_3_months"),
            \Carbon::now()
                ->addMonths(6)
                ->format("Y-m-d") => __("report.expiring_in_6_months"),
            \Carbon::now()
                ->addYear()
                ->format("Y-m-d") => __("report.expiring_in_1_year"),
        ];

        return view("report.stock_expiry_report")->with(
            compact(
                "categories",
                "brands",
                "units",
     ...
     * @return \Illuminate\Http\Response
     */
    public function getStockExpiryReportEditModal(
        Request $request,
                $purchase_line_id
    ) {
        if (
        ...
        if ($request->ajax()) {
            ...
            if (!empty($purchase_line)) {
                if (!empty($purchase_line->exp_date)) {
                    $purchase_line->exp_date = date(
                        "m/d/Y",
                        strtotime($purchase_line->exp_date)
                    );
                }
            }

            return view("report.partials.stock_expiry_edit_modal")->with(
                compact("purchase_line")
            );
        }
    }
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                if (!empty($purchase_line) && !empty($input["exp_date"])) {
                    $purchase_line->exp_date = $this->productUtil->uf_date(
                        $input["exp_date"]
                    );
                    $purchase_line->save();
                }
    ...
    public function getCustomerGroup(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Transaction::leftjoin(
                ...
                "transactions.customer_group_id",
                "=",
                "CG.id"
            )
                ->where("transactions.business_id", $business_id)
        ...
        return view("report.customer_group")->with(
            compact("customer_group", "business_locations")
        );
    }
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                ->select(
                    ...
                    DB::raw(
                        "(purchase_lines.quantity - purchase_lines.quantity_returned) as purchase_qty"
                    ),
                    "purchase_lines.quantity_adjusted",
                    "u.short_name as unit",
                    DB::raw(
                        "((purchase_lines.quantity - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted) * purchase_lines.purchase_price_inc_tax) as subtotal"
                    )
                )
            ...
            return Datatables::of($query)
                ...
                ->editColumn("purchase_qty", function ($row) {
                    return '<span data-is_quantity="true" class="display_currency purchase_qty" data-currency_symbol=false data-orig-value="' .
                ...
                ->editColumn("quantity_adjusted", function ($row) {
                    return '<span data-is_quantity="true" class="display_currency quantity_adjusted" data-currency_symbol=false data-orig-value="' .
                ...
                ->editColumn("unit_purchase_price", function ($row) {
                    return $this->transactionUtil->num_f(
                ...
                ->editColumn(
                    "supplier",
                    '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$supplier}}'
                )
                ->rawColumns([
                    "ref_no",
                    "unit_purchase_price",
        ...
        return view("report.product_purchase_report")->with(
            compact("business_locations", "suppliers", "brands")
        );
    }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        $product_custom_field1 = !empty(
        $custom_labels["product"]["custom_field_1"]
        )
            ? $custom_labels["product"]["custom_field_1"]
            : "";
        $product_custom_field2 = !empty(
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn("unit_sale_price", function ($row) {
                    return '<span class="unit_sale_price" data-orig-value="' .
                        $row->unit_sale_price .
                        '">' .
                        $this->transactionUtil->num_f(
                            $row->unit_sale_price,
                ...
                ->editColumn("unit_price", function ($row) {
                    return '<span class="unit_price" data-orig-value="' .
                        $row->unit_price .
                        '">' .
        ...
        return view("report.product_sell_report")->with(
            compact(
                "business_locations",
                "customers",
                "categories",
    ...
    public function getLotReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query
                ->select(
                    ...
                    DB::raw(
                        "COALESCE(SUM(IF(tspl.sell_line_id IS NULL, 0, (tspl.quantity - tspl.qty_returned)) ), 0) as total_sold"
                    ),
                    DB::raw(
                        "COALESCE(SUM(IF(tspl.stock_adjustment_line_id IS NULL, 0, tspl.quantity ) ), 0) as total_adjusted"
                    ),
                    "products.type",
            ...
            return Datatables::of($products)
                ->editColumn("stock", function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="display_currency total_stock" data-currency_symbol=false data-orig-value="' .
                        (float) $stock .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                ...
                ->editColumn("total_sold", function ($row) {
                    if ($row->total_sold) {
                        return '<span data-is_quantity="true" class="display_currency total_sold" data-currency_symbol=false data-orig-value="' .
                ...
                ->editColumn("total_adjusted", function ($row) {
                    if ($row->total_adjusted) {
                        return '<span data-is_quantity="true" class="display_currency total_adjusted" data-currency_symbol=false data-orig-value="' .
                            (float) $row->total_adjusted .
                            '" data-unit="' .
                            $row->unit .
                            '" >' .
                            (float) $row->total_adjusted .
                    ...
                    if (!empty($row->exp_date)) {
                        ...
                        if ($carbon_now->diffInDays($carbon_exp, false) >= 0) {
                            return $this->productUtil->format_date(
                                ...
                                '<br><small>( <span class="time-to-now">' .
                                $row->exp_date .
                                "</span> )</small>";
                        } else {
                            return $this->productUtil->format_date(
                                ...
                                ' &nbsp; <span class="label label-danger no-print">' .
                                __("report.expired") .
                                '</span><span class="print_section">' .
                                __("report.expired") .
                                '</span><br><small>( <span class="time-from-now">' .
                                $row->exp_date .
                                "</span> )</small>";
                        }
                    } else {
        ...
        return view("report.lot_report")->with(
            compact("categories", "brands", "units", "business_locations")
        );
    }
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $contact_filter1 = !empty($supplier_id)
                ...
                : "";
            $contact_filter2 = !empty($supplier_id)
                ? "AND transactions.contact_id=$supplier_id"
                : "";

            $location_id = $request->get("location_id", null);

            $parent_payment_query_part = empty($location_id)
                ? "AND transaction_payments.parent_id IS NULL"
                : "";
            ...
            $query = TransactionPayment::leftjoin(
                ...
                function ($join) use ($business_id) {
                    $join
                        ->on("transaction_payments.transaction_id", "=", "t.id")
                        ->where("t.business_id", $business_id)
                ...
                ->where(function ($q) use (
                    ...
                    $q->whereRaw(
                        "(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('purchase', 'opening_balance')  $parent_payment_query_part $contact_filter1)"
                    )->orWhereRaw(
                        "EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('purchase', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)"
                    );
                })
            ...
            $end_date = $request->get("end_date");
            if (!empty($start_date) && !empty($end_date)) {
                $query->whereBetween(DB::raw("date(paid_on)"), [
                    $start_date,
                    $end_date,
                ]);
            }
            ...
            return Datatables::of($query)
                ...
                ->editColumn("method", function ($row) use ($payment_types) {
                    $method = !empty($payment_types[$row->method])
                        ? $payment_types[$row->method]
                        : "";
                    if ($row->method == "cheque") {
                        $method .=
                            "<br>(" .
                            __("lang_v1.cheque_no") .
                            ": " .
                            $row->cheque_number .
                            ")";
                ...
                ->editColumn("amount", function ($row) {
                    return '<span class="paid-amount" data-orig-value="' .
                        $row->amount .
                        '">' .
                        $this->transactionUtil->num_f($row->amount, true) .
                        "</span>";
                })
                ->addColumn(
                    "action",
        ...
        return view("report.purchase_payment_report")->with(
            compact("business_locations", "suppliers")
        );
    }
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $contact_filter1 = !empty($customer_id)
                ? "AND t.contact_id=$customer_id"
                : "";
            $contact_filter2 = !empty($customer_id)
                ? "AND transactions.contact_id=$customer_id"
                : "";
            ...
            $query = TransactionPayment::leftjoin(
                ...
                ->where(function ($q) use (
                    ...
                    $q->whereRaw(
                        "(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('sell', 'opening_balance') $parent_payment_query_part $contact_filter1)"
                    )->orWhereRaw(
                        "EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('sell', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)"
                    );
                })
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
            ...
            return Datatables::of($query)
                ...
                ->editColumn("amount", function ($row) {
                    $amount =
                        $row->is_return == 1 ? -1 * $row->amount : $row->amount;
        ...
        $customer_groups = CustomerGroup::forDropdown(
            $business_id,
            false,
            true
        );

        return view("report.sell_payment_report")->with(
            compact(
                "business_locations",
                "customers",
                "payment_types",
    ...
    public function getTableReport(Request $request)
    {
        ...
        return view("report.table_report")->with(compact("business_locations"));
    }
    ...
    public function getServiceStaffReport(Request $request)
    {
        ...
        return view("report.service_staff_report")->with(
            compact("business_locations", "waiters")
        );
    }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    "transaction_sell_lines.parent_sell_line_id",
                    DB::raw(
                        'DATE_FORMAT(t.transaction_date, "%Y-%m-%d") as formated_date'
                    ),
                    DB::raw(
                        "(SELECT SUM(vld.qty_available) FROM variation_location_details as vld WHERE vld.variation_id=v.id $vld_str) as current_stock"
                    ),
                    DB::raw(
                        "SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) as total_qty_sold"
                    ),
                    "u.short_name as unit",
                    DB::raw(
        ...
        if ($request->ajax()) {
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw(
                        "(SELECT SUM(vld.qty_available) FROM variation_location_details as vld WHERE vld.variation_id=transaction_sell_lines.variation_id $vld_str) as current_stock"
                    ),
                    DB::raw(
                        "SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) as total_qty_sold"
                    ),
            ...
            return Datatables::of($query)
                ->editColumn(
                    "category_name",
                    '{{$category_name ?? __("lang_v1.uncategorized")}}'
                )
                ->editColumn(
                    "brand_name",
                    '{{$brand_name ?? __("lang_v1.no_brand")}}'
                )
                ->editColumn("total_qty_sold", function ($row) {
                    return '<span data-is_quantity="true" class="display_currency sell_qty" data-currency_symbol=false data-orig-value="' .
                        (float) $row->total_qty_sold .
    ...
    public function productStockDetails()
    {
        ...
        if (!empty(request()->input("location_id"))) {
            $location = BusinessLocation::where("business_id", $business_id)
                ...
                ->first();
            $stock_details = $this->productUtil->getVariationStockMisMatch(
                $business_id,
                $variation_id,
                $location_id
            );
        ...
        return view("report.product_stock_details")->with(
            compact("stock_details", "business_locations", "location")
        );
    }
    ...
    public function serviceStaffLineOrders()
    {
        ...
        $query->select(
            ...
            DB::raw(
                'CONCAT(COALESCE(ss.first_name, ""), COALESCE(ss.last_name, "")) as service_staff'
            )
        );
        ...
        $datatable = Datatables::of($query)
            ...
            ->editColumn("line_discount_amount", function ($row) {
                $discount = !empty($row->line_discount_amount)
                    ? $row->line_discount_amount
                    : 0;

                if (
                    !empty($discount) &&
                    $row->line_discount_type == "percentage"
                ) {
                    $discount =
                        $row->unit_price_before_discount * ($discount / 100);
                }

                return '<span class="display_currency total-discount" data-currency_symbol="true" data-orig-value="' .
                    $discount .
                    '">' .
                    $discount .
                    "</span>";
    ...
    public function getProfit($by = null)
    {
        ...
        if ($by == "product") {
            $query
                ...
                ->addSelect(
                    DB::raw(
                        "IF(P.type='variable', CONCAT(P.name, ' - ', PV.name, ' - ', V.name, ' (', V.sub_sku, ')'), CONCAT(P.name, ' (', P.sku, ')')) as product"
                    )
                )
                ->groupBy("V.id");
        }
        ...
        if ($by == "invoice") {
            $query
                ->addSelect(
                    ...
                    "CU.name as customer",
                    DB::raw(
                        "SUM(V.default_purchase_price * transaction_sell_lines.quantity) as total_cost"
                    ) // Add total cost calculation
                )
                ->join("contacts as CU", "sale.contact_id", "=", "CU.id")
                ->join(
                    "variations as V",
                    "transaction_sell_lines.variation_id",
                    "=",
                    "V.id"
                ) // Join variations to get the default_purchase_price
                ->groupBy("sale.invoice_no");
        }

        if ($by == "date") {
            $query
                ->addSelect("sale.transaction_date")
                ->groupBy(DB::raw("DATE(sale.transaction_date)"));
        }

        if ($by == "day") {
            $results = $query
                ->addSelect(DB::raw("DAYNAME(sale.transaction_date) as day"))
            ...
            return view("report.partials.profit_by_day")->with(
                compact("profits", "days")
            );
        }
        ...
        if (in_array($by, ["invoice"])) {
            $datatable->editColumn("gross_profit", function ($row) {
                ...
                if ($row->discount_type == "percentage") {
                    $discount =
                        ($row->discount_amount * $row->total_before_tax) / 100;
                }

                $profit = $row->gross_profit - $discount;
                $html =
                    '<span class="gross-profit" data-orig-value="' .
                    $profit .
                    '" >' .
            ...
            $datatable->editColumn("gross_profit", function ($row) {
                return '<span class="gross-profit" data-orig-value="' .
                    $row->gross_profit .
                    '">' .
                    $this->transactionUtil->num_f($row->gross_profit, true) .
                    "</span>";
            });
        }
        ...
        if ($by == "product") {
            $datatable->filterColumn("product", function ($query, $keyword) {
                $query->whereRaw(
                    "IF(P.type='variable', CONCAT(P.name, ' - ', PV.name, ' - ', V.name, ' (', V.sub_sku, ')'), CONCAT(P.name, ' (', P.sku, ')')) LIKE '%{$keyword}%'"
                );
            });
        }
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin(
                ...
                ->leftJoin(
                    "transactions as stock_adjustment",
                    "SAL.transaction_id",
                    "=",
                    "stock_adjustment.id"
                )
            ...
            if (
                !empty(request()->purchase_start) &&
                !empty(request()->purchase_end)
            ) {
                $start = request()->purchase_start;
                $end = request()->purchase_end;
                $query
                    ->whereDate("purchase.transaction_date", ">=", $start)
            ...
            return Datatables::of($query)
                ...
                ->editColumn(
                    "purchase_date",
                    '{{@format_datetime($purchase_date)}}'
                )
                ->editColumn("purchase_ref_no", function ($row) {
                    $html =
                        $row->purchase_type == "purchase"
                            ? '<a data-href="' .
                            action(
                                [
                                    \App\Http\Controllers\PurchaseController::class,
                ...
                ->editColumn("purchase_price", function ($row) {
                    return '<span 
                    class="purchase_price" data-orig-value="' .
                        $row->purchase_price .
                        '">' .
                        $this->transactionUtil->num_f(
                            $row->purchase_price,
                ...
                ->editColumn(
                    "sell_date",
                    '@if(!empty($sell_line_id)) {{@format_datetime($sell_date)}} @else {{@format_datetime($stock_adjustment_date)}} @endif'
                )

                ->editColumn("sale_invoice_no", function ($row) {
                    $invoice_no = !empty($row->sell_line_id)
                        ? $row->sale_invoice_no
                        : $row->stock_adjustment_ref_no .
                        "<br><small>(" .
                ...
                ->editColumn("quantity", function ($row) {
                    $html =
                        '<span data-is_quantity="true" class="display_currency quantity" data-currency_symbol=false data-orig-value="' .
                        (float) $row->quantity .
                        '" data-unit="' .
                        $row->unit .
                        '" >' .
                    ...
                    if ($row->qty_returned > 0) {
                        $html .=
                            '<small><i>(<span data-is_quantity="true" class="display_currency" data-currency_symbol=false>' .
                            (float) $row->quantity .
                            "</span> " .
                            $row->unit .
                            " " .
                ...
                ->editColumn("selling_price", function ($row) {
                    $selling_price = !empty($row->sell_line_id)
                        ? $row->selling_price
                        : $row->stock_adjustment_price;

                    return '<span class="row_selling_price" data-orig-value="' .
                        $selling_price .
                        '">' .
                        $this->transactionUtil->num_f($selling_price, true) .
                        "</span>";
                })
                ...
                ->addColumn("subtotal", function ($row) {
                    ...
                    return '<span class="row_subtotal" data-orig-value="' .
                        $subtotal .
                        '">' .
                        $this->transactionUtil->num_f($subtotal, true) .
                        "</span>";
                })
                ->editColumn(
                    "supplier",
                ...
                ->filterColumn("sale_invoice_no", function ($query, $keyword) {
                    $query
                        ->where("sale.invoice_no", "like", ["%{$keyword}%"])
                        ->orWhere("stock_adjustment.ref_no", "like", [
                            "%{$keyword}%",
                        ]);
                })
        ...
        return view("report.items_report")->with(
            compact("suppliers", "customers", "business_locations")
        );
    }
    ...
    public function purchaseReport()
    {
        if (
            (!auth()
                ...
                !auth()
                    ->user()
                    ->can("view_own_purchase")) ||
            empty(config("constants.show_report_606"))
        ) {
            abort(403, "Unauthorized action.");
        }
        $business_id = request()
        ...
        if (request()->ajax()) {
            ...
            $purchases = Transaction::leftJoin(
                "contacts",
                "transactions.contact_id",
                "=",
                "contacts.id"
                ...
                ->select(
                    ...
                    "tax_amount",
                    DB::raw(
                        'DATE_FORMAT(transaction_date, "%Y/%m") as purchase_year_month'
                    ),
                    DB::raw(
                        'DATE_FORMAT(transaction_date, "%d") as purchase_day'
                    )
                )
            ...
            if (
                !empty(request()->input("payment_status")) &&
                request()->input("payment_status") != "overdue"
            ) {
                $purchases->where(
                ...
                $purchases
                    ...
                    ->whereRaw(
                        "IF(transactions.pay_term_type='days', DATE_ADD(transactions.transaction_date, INTERVAL transactions.pay_term_number DAY) < CURDATE(), DATE_ADD(transactions.transaction_date, INTERVAL transactions.pay_term_number MONTH) < CURDATE())"
                    );
            }
            ...
            return Datatables::of($purchases)
                ...
                ->editColumn("discount_amount", function ($row) {
                    $discount = !empty($row->discount_amount)
                        ? $row->discount_amount
                        : 0;
                ...
                ->addColumn("payment_year_month", function ($row) {
                    ...
                    if (!empty($row->payment_lines->first())) {
                        $year_month = \Carbon::parse(
                            $row->payment_lines->first()->paid_on
                        )->format("Y/m");
                    }
                ...
                ->addColumn("payment_method", function ($row) use (
                    $payment_types
                ) {
                    $methods = array_unique(
                        $row->payment_lines->pluck("method")->toArray()
        ...
        return view("report.purchase_report")->with(
            compact("business_locations", "suppliers", "orderStatuses")
        );
    }
    ...
    public function saleReport()
    {
        if (
            (!auth()
                ...
                !auth()
                    ->user()
                    ->can("view_own_sell_only")) ||
            empty(config("constants.show_report_607"))
        ) {
            abort(403, "Unauthorized action.");
        }
        ...
        return view("report.sale_report")->with(
            compact("business_locations", "customers")
        );
    }
    ...
    public function getStockValue()
    {
        ...
        $profit_margin = empty($closing_stock_by_sp)
            ? 0
            : ($potential_profit / $closing_stock_by_sp) * 100;
    ...
    public function activityLog()
    {
        ...
        if (request()->ajax()) {
            $activities = Activity::with(["subject"])
                ...
                ->select(
                    "activity_log.*",
                    DB::raw(
                        "CONCAT(COALESCE(u.surname, ''), ' ', COALESCE(u.first_name, ''), ' ', COALESCE(u.last_name, '')) as created_by"
                    )
                );
            ...
            if (!empty(request()->start_date) && !empty(request()->end_date)) {
                ...
                $end = request()->end_date;
                $activities
                    ->whereDate("activity_log.created_at", ">=", $start)
            ...
            if (!empty($subject_type)) {
                if ($subject_type == "contact") {
                    ...
                    $activities->whereHasMorph(
                        "subject",
                        Transaction::class,
                        function ($q) use ($subject_type) {
            ...
            return Datatables::of($activities)
                ...
                ->addColumn("subject_type", function ($row) use (
                    $transaction_types
                ) {
                    $subject_type = "";
                    if ($row->subject_type == \App\Contact::class) {
                        ...
                        $row->subject_type == \App\Transaction::class &&
                        !empty($row->subject->type)
                    ) {
                        $subject_type = isset(
                            $transaction_types[$row->subject->type]
                        )
                            ? $transaction_types[$row->subject->type]
                            : "";
                    } elseif (
                        $row->subject_type == \App\TransactionPayment::class
                    ) {
                ...
                ->addColumn("note", function ($row) use (
                    $statuses,
                    $shipping_statuses
                ) {
                    $html = "";
                    ...
                    if (!empty($row->subject->invoice_no)) {
                        $html .=
                            __("sale.invoice_no") .
                            ": " .
                            $row->subject->invoice_no .
                            "<br>";
                    }
                    if (
                        $row->subject_type == \App\Transaction::class &&
                        !empty($row->subject) &&
                        in_array($row->subject->type, ["sell", "purchase"])
                    ) {
                    ...
                    if ($row->description == "contact_deleted") {
                        $html .=
                            $row->getExtraProperty("supplier_business_name") ??
                            "";
                        $html .= "<br>";
                    }
                    ...
                    if (!empty($row->getExtraProperty("invoice_no"))) {
                        $html .=
                            __("sale.invoice_no") .
                            ": " .
                            $row->getExtraProperty("invoice_no");
                    }
        ...
        return view("report.activity_log")->with(
            compact("users", "transaction_types")
        );
    }
    ...
    public function gstSalesReport(Request $request)
    {
        if (
            !auth()
                ->user()
                ->can("tax_report.view") ||
            empty(config("constants.enable_gst_report_india"))
        ) {
            abort(403, "Unauthorized action.");
        }
        ...
        if ($request->ajax()) {
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    "transaction_sell_lines.parent_sell_line_id",
                    DB::raw(
                        "((transaction_sell_lines.quantity- transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as line_total"
                    )
                )
                ->groupBy("transaction_sell_lines.id");
            ...
            foreach ($taxes as $tax) {
                ...
                $datatable->addColumn($col, function ($row) use (
                    ...
                    if (
                        ...
                        $sub_tax_share =
                            $this->transactionUtil->calc_percentage(
                                $row->unit_price_after_discount,
                                $group_taxes[$row->tax_id]["sub_taxes"][
                                $tax["id"]
                                ]->amount
                            ) * $row->sell_qty;
                    ...
                    if ($sub_tax_share > 0) {
                        ...
                        return '<span class="' .
                            ...
                            $sub_tax_share .
                            '">' .
                            $this->transactionUtil->num_f($sub_tax_share) .
                            "</span>";
                    } else {
                        return "";
                    }
            ...
            return $datatable
                ->addColumn("taxable_value", function ($row) {
                    $taxable_value =
                        $row->unit_price_after_discount * $row->sell_qty;
                    //ignore child sell line of combo product
                    $class = is_null($row->parent_sell_line_id)
                        ? "taxable_value"
                        : "";
                    ...
                    return '<span class="' .
                        ...
                        $taxable_value .
                        '">' .
                        $this->transactionUtil->num_f($taxable_value) .
                        "</span>";
                })
                ->editColumn("invoice_no", function ($row) {
                    return '<a data-href="' .
                ...
                ->editColumn("unit_price", function ($row) {
                    return '<span data-orig-value="' .
                        $row->unit_price .
                        '">' .
                        $this->transactionUtil->num_f($row->unit_price) .
                        "</span>";
                })
                ->editColumn("line_total", function ($row) {
                    return '<span data-orig-value="' .
                        $row->line_total .
                        '">' .
                        $this->transactionUtil->num_f($row->line_total) .
                        "</span>";
                })
                ->editColumn("discount_amount", function ($row) {
                    $discount = !empty($row->discount_amount)
                ...
                ->editColumn(
                    "tax_percent",
                    '@if(!empty($tax_percent)){{@num_format($tax_percent)}}% @endif
                    '
                )
                ->editColumn(
                    "customer",
        ...
        return view("report.gst_sales_report")->with(
            compact("customers", "taxes")
        );
    }
    ...
    public function gstPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = PurchaseLine::join(
                ...
                ->select(
                    ...
                    "u.short_name as unit",
                    DB::raw(
                        "((purchase_lines.quantity- purchase_lines.quantity_returned) * purchase_lines.purchase_price_inc_tax) as line_total"
                    )
                )
                ->groupBy("purchase_lines.id");
            ...
            return $datatable
                ->addColumn("taxable_value", function ($row) {
                    $taxable_value =
                        $row->unit_price_after_discount * $row->purchase_qty;

                    return '<span class="taxable_value"data-orig-value="' .
                        $taxable_value .
                        '">' .
                        $this->transactionUtil->num_f($taxable_value) .
                        "</span>";
                ...
                ->addColumn("discount_amount", function ($row) {
                    $discount = !empty($row->discount_percent)
        ...
        return view("report.gst_purchase_report")->with(
            compact("suppliers", "taxes")
        );
    }
}