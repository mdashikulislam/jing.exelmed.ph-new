...
<br>
	...
	<td>
		...
		<input name="default_purchase_price_p" data-original="{{ $product->default_purchase_price_o }}" class="default_purchase_price_p" value="{{ $product->default_purchase_price ? $product->default_purchase_price : 'no'  }}" type="hidden">
	...
	@else
		...
		@endif
	@endif
	<td class="{{$hide_tax}} @if(auth()->user()->hasPermissionTo('hide_price')) hide @endif">
	...
	@endif
	<td class="text-center @if(auth()->user()->hasPermissionTo('hide_price')) hide @endif">
		@php
